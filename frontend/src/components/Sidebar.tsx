...
import {
  ...
  MessageSquare, Settings, CalendarCheck, X, Apple
} from 'lucide-react';
  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 h-full bg-gray-900 z-50 flex flex-col transition-all duration-300 ${
          ...
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            {(!isCollapsed || isMobileOpen) && (
              <div className="whitespace-nowrap transition-opacity duration-300">
                ...
                {isSuperAdmin && (
                  <div className="text-[10px] text-orange-400 font-semibold tracking-widest uppercase">SuperAdmin</div>
                )}
              </div>
            )}
          </div>
        ...
        <button
          onClick={() => setIsMobileOpen(false)}
          className="lg:hidden absolute top-4 right-4 p-2 text-gray-400 hover:bg-gray-800 rounded-lg"
        >
          <X size={20} />
        </button>
        ...
        <div className={`px-4 py-3 border-t border-gray-700/50 flex items-center ${(!isCollapsed || isMobileOpen) ? 'gap-3' : 'justify-center'}`}>
          <div className="w-9 h-9 min-w-[36px] rounded-full flex items-center justify-center text-white text-sm font-bold" style={{ background: 'hsl(24 95% 53%)' }}>
            {user?.name?.charAt(0)?.toUpperCase() || 'A'}
          </div>
          {(!isCollapsed || isMobileOpen) && (
            <div className="whitespace-nowrap overflow-hidden">
              <div className="text-white text-sm font-medium">{user?.name || 'Admin'}</div>
              <div className={`text-xs font-semibold ${isSuperAdmin ? 'text-orange-400' : 'text-gray-500'}`}>
                {isSuperAdmin ? '⭐ Super Admin' : user?.role || 'Gym Admin'}
              </div>
            </div>
          )}
        </div>
